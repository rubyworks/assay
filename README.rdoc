...
foundation, but in the end we want to write assertions more easily and
concisely. For this we turn to separate "grammer" projects that depend
on Assay's classes. The first of these, created as a spin-off project
to demonstrate Assay's prowess, is {Fluidity}[http://rubyworks.github.com/fluidity].
...
Fluidity is just one possible grammer. There is no reason not to build
your own grammer on top of Assay's classes if you have another approach.
Indeed, please do so! That, after all, is the main purpose of having such
a set of reusable assertion classes!

=== Compatibility Gems

Thre are a few spin-off projects that provide compatability modes for legacy
test frameworks which can server as transitions to Assay from these other
